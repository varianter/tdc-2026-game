...
func TestCollisionYAxisFromAbove(t *testing.T) {
	...
	// Align player horizontally inside platform: x so collision square overlaps [100,200]
	// collision square x = x+15, w=34, so x+15=133 => x=118
	p := NewTestSquare(118, 50)
	p.vy = -50.0 // falling down
	dt := 1.0 / 60
...
func NewTestSquare(x, y float64) *MovingSquare {
	collisionOffset, w, h, vx, vy, direction := 15.0, 64.0, 64.0, 90.0, 0.0, 1.0
	p := &MovingSquare{
